...
Every OS workflow that may change goals, priorities, objectives, success definitions or other strategic direction must run the strategic-write assertion first.
...
- OS reads `.aiverse/direction/views/<scope>.md` and Brain refs for strategic display/routing.
- OS may continue editing operational/current-state fields such as current facts, next useful actions, pending decisions, source pointers, connection state and execution state.

A Brain outage does **not** transfer ownership. The ownership marker is independent of Brain process availability, so OS strategic writes remain blocked until an explicit future ownership-transfer protocol changes the marker.
...
The original OS files are not deleted during handover. They are frozen as provenance after Brain ownership becomes active.
...
- action permissions;
- scheduler ownership;
- current-state/source-of-truth routing.
