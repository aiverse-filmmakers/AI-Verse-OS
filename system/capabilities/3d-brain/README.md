...
Build a named, interactive globe from an AI-Verse OS's saved knowledge. The skill asks for the brain's display name and main categories, maps them to selected local folders, and creates the bundled app under `apps/3d-brain/`.

- Claude Code: `/3d-brain`
- Codex: select **3D Brain**, or type `$3d-brain`
- Natural language: “Build a 3D brain from my AI-Verse OS.”
...
To install separately, copy this entire folder, including `assets`, `scripts`, `references`, and `agents`, to the target AI-Verse OS's `.claude/skills/3d-brain/`. In AI-Verse OS, run `bash scripts/sync-codex-skills.sh 3d-brain` to generate the Codex copy. Do not copy only `SKILL.md` or include a generated user's config/data.
