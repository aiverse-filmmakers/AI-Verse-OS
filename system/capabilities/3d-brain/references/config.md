...
This is a schema example, not permission to assume those folders exist. Include only verified and chosen paths. The scaffold computes `root` relative to the generated app, so the AI-Verse OS can move between folders or machines without breaking its in-repo paths. A local `brain.config.json` is gitignored.
...
| `root` | AI-Verse OS root relative to the installed app's config. Scaffold sets this automatically. |
...
| `sources[].paths` | Approved files/folders, relative to the AI-Verse OS root or explicit absolute/`~/` paths. No glob patterns. |
